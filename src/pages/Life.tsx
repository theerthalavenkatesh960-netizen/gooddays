...
function ReviewTab() {
  ...
  return (
    <div className="px-4">
      ...
      {snapshots.length > 0 && (
        <div className="mb-5">
          <p className="section-label mb-3">Recommendation History</p>
          <div className="space-y-2">
            {snapshots.slice(0, 5).map(s => (
              <div key={s.id} className="flex items-center justify-between rounded-xl px-3 py-2" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }}>
                ...
                <span className="text-[11px] px-2 py-0.5 rounded-full font-semibold" style={{
                  backgroundColor: s.status === 'approved' ? 'var(--accent-green)22' : s.status === 'dismissed' ? 'var(--surface-elevated)' : s.status === 'partial' ? 'var(--accent)22' : 'var(--accent)22',
