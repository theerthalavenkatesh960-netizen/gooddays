...
function DashboardTab({
  ...
  return (
    <div>
      {/* Today progress hero */}
      <div className="mb-4 p-4 rounded-2xl" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--accent)55' }}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: 'var(--accent)' }}>Today’s Progress</p>
            <p className="text-3xl font-extrabold num mt-0.5" style={{ color: 'var(--text-primary)' }}>{loading ? '--' : todayProgress.completed}<span className="text-lg font-semibold" style={{ color: 'var(--text-muted)' }}>/{loading ? '--' : todayProgress.total}</span></p>
          ...
          <div className="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: `conic-gradient(var(--accent) ${todayProgressPercent * 3.6}deg, var(--border) 0deg)` }}>
            <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--surface)' }}>
        ...
        <div className="mt-3 h-2 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--border)' }}>
          <div className="h-full rounded-full transition-all" style={{ width: loading ? '35%' : `${todayProgressPercent}%`, backgroundColor: 'var(--accent)' }} />
        </div>
      ...
      <div className="mb-4 rounded-2xl p-4" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>7-day activity</p>
            <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>A quick read of your consistency</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 gap-1.5 overflow-x-auto">
            {activityDays.map((day) => {
              ...
              return (
                <div key={day.key} className="min-w-[42px] rounded-lg px-1 py-2 text-center" style={{ backgroundColor: day.key === today ? 'var(--accent)18' : 'var(--surface-elevated)' }} title={day.dateLabel}>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {[
          { label: 'Task consistency', value: `${weeklyTaskDays}/7 days`, icon: CheckSquare, onClick: onOpenTasks, progress: weeklyTaskDays / 7 },
          { label: 'Workout consistency', value: `${weeklyWorkoutDays}/7 days`, icon: Dumbbell, onClick: () => navigate('/body?tab=Workout'), progress: weeklyWorkoutDays / 7 },
          { label: 'Journal days', value: `${weeklyJournalDays}/7 days`, icon: BookOpen, progress: weeklyJournalDays / 7 },
          { label: 'Weekly spend', value: formatMoney(weeklySpend), icon: TrendingUp, onClick: () => navigate('/finance') },
          { label: 'Monthly spend', value: formatMoney(monthlySpend), icon: DollarSign, onClick: () => navigate('/finance'), detail: spendDelta === null ? 'No prior month comparison' : `${spendDelta > 0 ? '+' : ''}${spendDelta}% vs last month` },
          { label: 'Goals', value: `${goalsInProgress} active`, icon: Target, onClick: () => navigate('/goals'), detail: nearestGoalDays === null ? 'No upcoming deadline' : `${goalUrgency} · ${nearestGoalDays}d`, detailColor: goalUrgencyColor },
        ].map((item) => {
          const Icon = item.icon;
          return (
            <button key={item.label} onClick={item.onClick} disabled={!item.onClick} className="rounded-xl p-3 text-left press" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--border)', opacity: item.onClick ? 1 : 0.85 }}>
              ...
              {item.progress !== undefined && <div className="h-1 mt-2 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--border)' }}><div className="h-full rounded-full" style={{ width: `${item.progress * 100}%`, backgroundColor: 'var(--accent)' }} /></div>}
